...
export default function BlogPage() {
  ...
  return (
    <main style={{ background: "#080c12", minHeight: "100vh", color: "#fff", fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif" }}>
      ...
      <section style={{ maxWidth: "760px", margin: "0 auto", padding: "120px 48px 100px" }}>
            ...
            <Link key={post.slug} href={`/blog/${post.slug}`} style={{ textDecoration: "none" }}>
              <div style={{ padding: "28px 0", borderBottom: i < posts.length - 1 ? "0.5px solid rgba(255,255,255,0.06)" : "none", display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "24px" }}>
                <div style={{ flex: 1 }}>
                  ...
                  {post.excerpt && <div style={{ color: "rgba(255,255,255,0.3)", fontSize: "13px", lineHeight: 1.7, marginBottom: "10px" }}>{post.excerpt}</div>}
                  <div style={{ color: "rgba(255,255,255,0.2)", fontSize: "12px" }}>{post.date} · {post.readTime} read</div>
                </div>
